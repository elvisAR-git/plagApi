...
let fileModel = require("../models/files.model");
...
const minimum_similarity = 0.9;

module.exports = async function plagchecker(fileIn) {
  const readFile = util.promisify(fs.readFile);
  let analysis = [];
  let result = await readFile(config.UPLOAD + fileIn.name);
  let someArray = [];
  ...
  for (const file of existing_files) {
    ...
    if (file._id != fileIn._id) {
      try {
        let result = await readFile(config.UPLOAD + file.name);
          ...
          file_lines.forEach((line2) => {
            ...
            if (line2 != "" && line1 != "" && regExp.test(line1)) {
              ...
              if (similarity >= minimum_similarity) {
                ...
                if (
                  !plagiarised_lines.some(
                    (el) => el.number === plag_line.number
                  )
                ) {
